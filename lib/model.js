...
exports.Model = function(tableName, adapter){
	...
	self.query = adapter.connection.query;
	...
	this.all = function(callback){
		this.query("SELECT * from " + tableName + ";", {}, function (error, result) {
	...
	this.first = function (callback) {
		this.query("SELECT * from " + tableName + " limit 1;", {}, function (error, result) {
			callback && callback(error, activateRow(result && result.rows[0]));
		})		
	}
	this.last = function (callback) {
		this.query("SELECT * from " + tableName + " order by " + this.primaryKey + " desc limit 1;", {}, function (error, result) {
			callback && callback(error, activateRow(result && result.rows[0]));
		})		
	}
	...
	this.find = function (id, callback, options) {
		...
		this.query("SELECT * from " + tableName + " where ( " + params.keys + " ) = ( "+ params.values +" );", {}, function (error, result) {
			callback && callback(error, activateRow(result && result.rows[0]));
		})
	};
	...
	this.where = function (query, callback) {
		this.query('SELECT * from ' + tableName + ' where ' + query + ";", {}, function (error, result) {
