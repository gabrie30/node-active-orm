...
module.exports = function(tableName, adapter){
	var self = this;
	var adapter = adapter;
	var primaryKey = "id";
	var validators = [];
	...
	var activateRow = function(row){
		var record = row ? new ActiveRecord( row , self ) : null;
		return record;
	}
	...
	self.__defineGetter__("primaryKey", function(){ return primaryKey });
	...
	this.update = function(id, params , callback){
		var activeRecord = new ActiveRecord(params,this);
	...
	this.create = function (params, callback, force) {
		...
		if( this.isValid(activeRecord) ){
			...
			this.connection.query( query , {}, function (error, result) {
				var activeRecord = result && result.rows[0] && ( new ActiveRecord(result.rows[0], self ));
				callback && callback(error, activeRecord);
